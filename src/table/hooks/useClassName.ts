...
let globalClassNames: TableClassName;
let lastClassPrefix = 't';

function getClassNames(classPrefix: string) {
  const classNames = {
    ...
    tableFooterClasses: {
      ...
      footerSummary: `${classPrefix}-table__footer-summary`,
...
export default function useClassName() {
  ...
  const getGlobalClassNames = () => {
    if (!globalClassNames || lastClassPrefix !== classPrefix.value) {
      globalClassNames = getClassNames(classPrefix.value);
    }
    return globalClassNames;
  };
  
  const classNames = ref<TableClassName>(getGlobalClassNames());

  watch(classPrefix, () => {
    classNames.value = getGlobalClassNames();
  });

  return classNames.value;
}

export type TableClassName = ReturnType<typeof getClassNames>;
